...
class ExperimentEngine(object):
    ...
    def todo(self, maxiter=0):
        for i in itertools.count():
            if maxiter != 0 and i == maxiter:
                return
            got = False
            for e in self.experiments:
                logfile = "{}/{}-{}".format(self.logdir, e.name, i)
                status, value = e.get_status(logfile)
                if status == Experiment.DONE:
                    got = True
                elif status == Experiment.TIMEOUT:
                    got = True
                elif (os.path.isfile(logfile)):
                    print("{}: not done.".format(e.name))
                else:
                    print("{}: to do.".format(e.name))
            if not got:
                return

    def dry_run(self, maxiter=0):
        for i in itertools.count():
            if maxiter != 0 and i == maxiter:
                return
            got = False
            for e in self.experiments:
                ...
                status, value = e.get_status(logfile)
                if status == Experiment.DONE:
                    print("{}: {}.".format(e.name, e.get_text(value)))
                    got = True
                elif status == Experiment.TIMEOUT:
                    print("{}: timeout.".format(e.name))
                    got = True
                elif (os.path.isfile(logfile)):
                    print("{}: not done.".format(e.name))
            if not got:
    ...
    def get_status(self, e, i):
        logfile = "{}/{}-{}".format(self.logdir, e.name, i)
        status, value = e.get_status(logfile)
        return status, value

    def get_results(self, maxiter=0):
        """Get all results.
        """
    ...
    def report_group(self, group, maxiter=None):
        # create directory
        if not os.path.exists(self.logdir):
            return

        # get experiments...
        exps = [x for x in self.experiments if str(x.group) == str(group)]

        for i in itertools.count():
            if maxiter is not None and maxiter <= i: return
            got = False
            for e in exps:
                logfile = "{}/{}-{}".format(self.logdir, e.name, i)
                status, value = e.get_status(logfile)
    ...
    def run_group(self, group, maxiter=None):
        # create directory
        if not os.path.exists(self.logdir):
        ...
        print("Found {} experiments in group {}".format(len(exps), group))

        for i in itertools.count():
            if maxiter is not None and maxiter <= i: break
            random.shuffle(exps)
            for e in exps:
                logfile = "{}/{}-{}".format(self.logdir, e.name, i)
                status, value = e.get_status(logfile)
                if status == Experiment.DONE: continue
                if status == Experiment.TIMEOUT and value >= self.timeout: continue
                # run experiment
                print("It {}: ".format(i), end='')
                status, value = e.run_experiment(self.timeout, logfile)
                # sleep 1 second to allow OS to swap stuff in or out
                time.sleep(1)

    def get_groups(self):
        return list(set([x.group for x in self.experiments]))

    def run_experiments(self, maxiter=None):
        ...
        for i in itertools.count():
            if maxiter is not None and maxiter <= i:
                break
            if i < n_iterations:
                todo = self.notdone[i]
                self.notdone[i] = []
