...
class ExpOinkAPT(ExpOink):
    def __init__(self, name, model):
        self.group = name
        self.solver = "apt"
        self.name = "{}-apt".format(name)
        self.call = ["tools/oink", "--apt", "-v", model]
        self.model = model
...
class DirectoryExperiments(Experiments):
    def __init__(self, directory, forbidden=[]):
        ...
        self.models = self.models + [(x, "{}/{}.pg".format(directory, x)) for x in files]
        files = list(filter(lambda f: os.path.isfile(directory+"/"+f), os.listdir(directory)))
        ...
        files = list(filter(lambda f: os.path.isfile(directory+"/"+f), os.listdir(directory)))
        files = [f[:-len(".gm.gz")] for f in filter(lambda f: f.endswith(".gm.gz"), files)]
        self.models = self.models + [(x, "{}/{}.gm.bz2".format(directory, x)) for x in files]
        files = list(filter(lambda f: os.path.isfile(directory+"/"+f), os.listdir(directory)))
        files = [f[:-len(".pg.gz")] for f in filter(lambda f: f.endswith(".pg.gz"), files)]
        self.models = self.models + [(x, "{}/{}.gm.bz2".format(directory, x)) for x in files]
        files = list(filter(lambda f: os.path.isfile(directory+"/"+f), os.listdir(directory)))
        files = [f[:-len(".gm.bz2")] for f in filter(lambda f: f.endswith(".gm.bz2"), files)]
        self.models = self.models + [(x, "{}/{}.gm.bz2".format(directory, x)) for x in files]
        files = list(filter(lambda f: os.path.isfile(directory+"/"+f), os.listdir(directory)))
        files = [f[:-len(".pg.bz2")] for f in filter(lambda f: f.endswith(".pg.bz2"), files)]
        self.models = self.models + [(x, "{}/{}.pg.bz2".format(directory, x)) for x in files]
...
class OinkExperiments(DirectoryExperiments):
    def get_solvers(self):
        return {
            ...
            #'atl-n': lambda name, filename: ExpOinkATL(name, filename).nosp(),
            'otftl': lambda name, filename: ExpOinkOTFTL(name, filename),
            #'otftl-n': lambda name, filename: ExpOinkOTFTL(name, filename).nosp(),
            'otfatl': lambda name, filename: ExpOinkOTFATL(name, filename),
            #'otfatl-n': lambda name, filename: ExpOinkOTFATL(name, filename).nosp(),
            #'apt': lambda name, filename: ExpOinkAPT(name, filename),
            #'apt-n': lambda name, filename: ExpOinkAPT(name, filename).nosp(),
            ...
            #'rr-n': lambda name, filename: ExpOinkRR(name, filename).nosp(),
            # 'dp': lambda name, filename: ExpOinkDP(name, filename),
            #'rrdp': lambda name, filename: ExpOinkRRDP(name, filename),
            #'rrdp-n': lambda name, filename: ExpOinkRRDP(name, filename).nosp(),
...
sr = ExperimentEngine(logdir='logs_tl', timeout=default_timeout)
...
#   give list of experiments that are not don
# exp.py dryrun (ITERATIONS=1)
#   what?
# exp.py cull1
# exp.py cull2
# exp.py csv
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        if sys.argv[1] == 'rungroup':
            ...
            if len(sys.argv) > 2:
                sr.dry_run(int(sys.argv[2]))
            else:
                sr.dry_run(1)  # by default just 1 iteration
        ...
        elif sys.argv[1] == 'csvt':
            # report 1st iteration
            for group in set([exp.group for exp in sr.experiments]):
                for e in [exp for exp in sr.experiments if exp.group == group]:
                    status, value = sr.get_status(e, 0)
                    if status == Experiment.DONE and "tangles" in value:
                        whichset = e.model[:e.model.find("/")]
                        print("{};{};{};{:.6f};{};{};{};{}".format(whichset, group, e.solver, value["time"], value["tangles"], value["nodes"], value["edges"], value["priorities"]))
